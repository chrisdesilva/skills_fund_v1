...
const Apply = () => {
	...
	const [ loanValue, setLoanValue ] = useState(schoolList[programIndex]['node']['loanInfo'][0]['defaultAmount']);
	...
	useEffect(
		() => {
			...
			setProgramIndex(0);
			setLoanInformation(schoolList[schoolIndex]['node']['loanInfo'][programIndex]);
		...
		<Layout>
			...
			<ApplyContainer>
				<ApplyCard>
					...
					<SelectContainer showProgram={showProgram}>
						...
						<select id="program" defaultValue={'default'} onChange={selectProgram}>
							<option disabled value="default">
								---
							</option>
					...
					<Form ready={submitReady} showForm={showForm} onSubmit={handleSubmit}>
						<input type="email" placeholder="Enter your email address" required onChange={handleEmail} />
						<input
							type="submit"
							value="Next &rarr;"
			...
			<CalculatorContainer showProgram={showProgram}>
				<CalculatorCard showProgram={showProgram}>
					...
					<SelectContainer showProgram={showProgram}>
						<label htmlFor="program">Select your program</label>
						<select id="program" defaultValue={'default'} onChange={selectProgramLoan}>
							<option disabled value="default">
								---
							</option>
					...
					{loanInformation && (
						<div className="loanCalculator__slider">
							...
							<div className="loanCalculator__labels">
								...
								<p>
									Loan Amount<br />
									<span className="loanCalculator__amount">{formatter.format(loanValue)}</span>
								</p>
								<p>{formatter.format(loanInformation.aprAndType[0].maxTuition)}</p>
							</div>
						</div>
					)}
				</CalculatorCard>
...
export const CalculatorCard = styled.div`
  ...
  height: ${({ showProgram }) => (showProgram ? '292' : '0')};
	opacity: ${({ showProgram }) => (showProgram ? '1' : '0')};
  transform: ${({ showProgram }) => (showProgram ? 'translateY(0)' : 'translateY(-5%)')};
	z-index: -10:
	width: 40rem;
	padding: 2rem;
	border: 1px solid lightgray;
